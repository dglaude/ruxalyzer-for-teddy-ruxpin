...
# Based on https://stackoverflow.com/questions/57252419/how-to-modify-the-pixel-value-of-each-pixel-conditionally-using-pil-image-only
def getCorrectedImage(img):
    data = list(img.getdata())

# This check if RED component is zero, and if so put the Blue value as pure red shade, else keep color:
    new_data = [(x[2], 0, 0)  if x[0]==0 else (x[0], x[1], x[2]) for x in data] # If RED is zero take blue to make RED
